...
- **流派与智能列表**：可按流派浏览，并访问收藏、最近添加、最近播放、最常播放和随机专辑等服务器列表。
- **服务器播放列表**：可浏览服务器播放列表，将当前 foobar2000 播放列表创建或更新到服务器，并重命名或删除服务器播放列表。
- **下载原始文件**：可从浏览器右键菜单下载服务器保存的原始音频文件，不受播放转码设置影响。
- **播放格式设置**：可选择服务器默认、原始格式、MP3、Opus、AAC、FLAC 或 WAV，并限制最高码率。
...
- **Enter 快捷播放**：在浏览器中按 Enter 会先清空当前活动播放列表，再加入所选内容并开始播放。
...
6. 按需选择流格式和最高码率；选择“原始格式”可禁用服务器转码。
7. 点击 **测试连接**；连接成功后点击 **应用** 或 **确定**。
...
- 右键歌曲、专辑、艺术家、流派或播放列表，可下载其中的原始文件。
- 在“服务器播放列表”节点右键可同步当前播放列表；右键具体服务器播放列表可重命名或删除。
- 按 Enter 可替换当前活动播放列表并立即播放所选内容。
